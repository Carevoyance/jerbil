...
var CRLF = new Buffer('\r\n');
var DEFAULT_RECONNECT_DELAY = 10;
...
var Generic = exports.Generic = function (_EventEmitter) {
  ...
  function Generic(port, host) {
    ...
    _this.userDisconnected = false;
    _this.connected = false;
    _this.raw = false;
    _this.setMaxListeners(0);
    _this.queue = new _doubleEndedQueue2.default();
    _this.reconnectTimer = DEFAULT_RECONNECT_DELAY;
    return _this;
  }
  ...
  _createClass(Generic, [{
    ...
    value: function connect() {
      ...
      this.conn.on('connect', function () {
        ...
        _this2._handleConnect(callback);
      });
    ...
    value: function _handleResponse(responseData) {
      ...
      var handleBatchResponse = function handleBatchResponse() {
        if (responseData.indexOf(CRLF, separatorIndex + CRLF.length) !== -1) {
          // Continue processing batch response
          _this4._handleResponse(responseData.slice(separatorIndex + CRLF.length));
        }
      };
    ...
    value: function sendCommand(args, callback) {
      ...
      if (!this.connected) {
        return this.once('connect', function () {
          return _this5.sendCommand(args, callback);
        });
      }
...
var Worker = exports.Worker = function (_Generic) {
  ...
  function Worker(port, host) {
    ...
    var _this6 = _possibleConstructorReturn(this, (Worker.__proto__ || Object.getPrototypeOf(Worker)).call(this, port, host));

    _this6.tubes = new Set();
    return _this6;
  }
  ...
  _createClass(Worker, [{
    ...
    value: function _handleConnect(callback) {
      ...
      var nextTube = function nextTube() {
        ...
        if (tube.done) {
          return _get(Worker.prototype.__proto__ || Object.getPrototypeOf(Worker.prototype), '_handleConnect', _this7).call(_this7, callback);
        }

        _this7.sendCommand(_this7._makePrioritized(['watch', tube.value]), function (err) {
          if (err) {
    ...
    value: function ignore(tube, callback) {
      ...
      this.sendCommand(['ignore', tube], function (err, count) {
        if (err) {
          callback.call(_this9, err);
        } else {
          _this9.tubes.delete(tube);
          callback.call(_this9, err, count);
        }
      });
    }
  }]);
...
var Producer = exports.Producer = function (_Generic2) {
  ...
  function Producer(port, host) {
    ...
    var _this10 = _possibleConstructorReturn(this, (Producer.__proto__ || Object.getPrototypeOf(Producer)).call(this, port, host));

    _this10.tube = undefined;
    return _this10;
  }
  ...
  _createClass(Producer, [{
    ...
    value: function _handleConnect(callback) {
      ...
      this.sendCommand(this._makePrioritized(['use', this.tube]), function (err) {
        ...
        _get(Producer.prototype.__proto__ || Object.getPrototypeOf(Producer.prototype), '_handleConnect', _this11).call(_this11, callback);
      });
    }
    ...
    value: function use(tube, callback) {
      ...
      this.sendCommand(['use', tube], function (err, tube) {
        if (err) {
          callback.call(_this12, err);
        } else {
          // Stash tube for reconnects
          _this12.tube = tube;
          callback.call(_this12, err, tube);
        }
      });
    }
  }, {
